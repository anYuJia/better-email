...
- 前端组件按 `Sidebar`、`MessageListPane`、`ReaderPane`、写信、设置和弹层继续拆分；快捷键帮助、命令面板和撤销提示栈分别由 `ShortcutHelpModal`、`CommandPalette`、`UndoSnackbarStack` 管理，领域类型、纯配置和通用交互 Hook 脱离主组件，减少单文件状态和 JSX 体积。
