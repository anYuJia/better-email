...
- 设置窗口使用固定标题栏、分类侧栏和单一内容滚动区；“连接测试”“保存设置”作为当前账号的全局主操作常驻标题栏，避免用户修改账号后滚动查找按钮；备份、诊断、同步和安全预览仍留在各自分类。600px 以下主操作收为 32px 图标按钮并保留 `aria-label`/tooltip，不挤压标题。
...
- `src/`：前端界面、状态管理、列表/阅读/撰写交互；主界面已拆分 `src/components/Sidebar.tsx`、`AccountSwitcher.tsx`、`SidebarFolderNavigation.tsx`、`MessageListPane.tsx`、`ReaderPane.tsx`，账号范围弹出菜单和文件夹固定/拖拽/右键职责分别由独立组件承载；设置窗口框架、分类导航和顶部主操作位于 `src/components/settings/SettingsFrame.tsx`，账号/认证、体验、数据、同步、联系人、规则和安全预览继续按领域组件拆分；联系人领域流程位于 `src/hooks/useContactManagement.ts`，OAuth2 设置与授权流程位于 `src/hooks/useOAuthFlow.ts`，轻量邮件拖拽协议位于 `src/components/messageDrag.ts`，领域类型和纯配置位于 `src/app/`。
