...
| 大正文的前端开销 | 大型正文先显示明确的加载入口，用户可访问完整内容；摘要限长与正文分离 | ReaderBodyContent.tsx |
...
| 旧草稿请求覆盖新内容、清除后复活 | 版本条件写入、单实例请求串行化、清除墓碑，数据库确认优先于旧 localStorage | useComposerRecovery.ts、composerRecovery.ts |
...
样式整理不是全站重新设计；根组件提取也不等于全应用完成状态机重写。`lint` 仍为 TypeScript 检查，本 PR 没有新增 ESLint 依赖或声称 ESLint 门禁已存在。收发诊断、附件体验与设置分类的进一步产品重构仍需独立设计验收。
...
npm run test:ui
node scripts/benchmark-list.mjs list-benchmark.json
...
本地已执行前端完整回归、类型检查、构建、架构/CSS/体积检查。原生测试、Clippy 与真实浏览器结果在 PR/Actions 留证。运行过程中暴露的失败不能删除测试或绕过门禁，应修复或明确保留 Draft。
...
原生设备冷启动、真实滚动帧率、长期内存增长、多账号真实同步/搜索负载仍需要固定设备测量；列表算法结果不能代替这些指标。AI/MCP 外发确认、备份导入导出、服务商 OAuth 与系统安全边界仍需端到端安全验收，不能宣称安全审计通过。

草稿恢复点目前是应用级单槽，跨窗口冲突会拒绝覆盖并提示，尚未改造成多草稿协同恢复系统。生命周期事件上的异步保存是尽力执行，不能保证系统强制终止前一定落盘。收件网络路径没有全部改写为异步协议客户端。
