...
- 附件转发：阅读面提供紧凑的附件总数、总大小和顺序“下载全部”，避免多个大附件并发推高内存；批量下载逐项执行，单项失败保留断点和重试状态并继续后续项目。转发草稿只携带已完整下载且具有有效本地路径的原附件，未下载项明确提示数量；原邮件正文含常见 HTML 结构时，引用内容回退到安全纯文本摘要，避免标签进入撰写器。
...
- 邮件解析：MIME、多部分正文、HTML 安全渲染、内嵌图片、附件；附件按 MIME part 分段下载，BODYSTRUCTURE、整封回退和分段请求最多自动尝试 3 次，失败间隔为 150 ms、300 ms。编码后的 `.download` 临时文件在 100 MB 安全范围内跨失败和应用重启保留，下次以文件长度作为 IMAP partial offset；完成后按 BODYSTRUCTURE 流式解码 Base64、Quoted-Printable 或直通内容，最终解码文件继续受 25 MB 上限保护。完成前不写入已下载状态，成功后原子重命名，阅读面持续提供进度、失败原因、手动重试和顺序下载全部入口；转发只复用已完成下载的本地附件。
- 组织方式：文件夹、标签、规则、智能收件箱、会话线程。
- 搜索：SQLite FTS5，本地索引，发件人/日期/附件过滤。
- 安全：凭据存系统 Keychain，远程图片默认阻止，HTML 清洗，钓鱼提示。
- 通知：系统通知、角标、免打扰、VIP 发件人、账号静音、会话静音和重点账号提醒；通知摘要只统计策略过滤后仍会触发提醒的邮件。
...
- 下拉框、弹窗、菜单、复选框和输入区统一 9–13px 控件圆角、轻边框、明确悬停/焦点/危险状态；设置中心另有 `settings-design-language.css` 作为最后加载的局部视觉覆盖层，负责 2026 风格的页头进度、卡片、移动端选择器和分页按钮；最终 `ui-2026.css` 固定在全部全局组件样式之后加载，避免组件局部样式覆盖最终视觉层；弹层必须留在当前窗体内并高于内容层，纯图标关闭按钮必须保留 `aria-label` 和 tooltip，状态提示使用右下角紧凑浮层，不覆盖邮件列表。
...
- `src/`：前端界面、状态管理、列表/阅读/撰写交互；主界面已拆分 `src/components/Sidebar.tsx`、`AccountSwitcher.tsx`、`SidebarFolderNavigation.tsx`、`MessageListPane.tsx`、`MessageListToolbar.tsx`、`MessageBulkToolbar.tsx`、`MessageListView.tsx`、`ThreadListView.tsx`、`ReaderPane.tsx`，账号范围弹出菜单和文件夹固定/拖拽/右键职责分别由独立组件承载；设置窗口框架、分类导航和顶部主操作位于 `src/components/settings/SettingsFrame.tsx`，最终设置视觉覆盖位于 `src/components/settings/settings-design-language.css`，账号移除危险区和确认弹窗位于 `AccountRemovalPanel.tsx`，安全凭据输入与诊断位于 `CredentialSecuritySettings.tsx`，写入验收和回写步骤位于 `ProviderWriteValidationSettings.tsx`、`ProviderWritebackValidationPanel.tsx`，账号/认证、体验、数据、同步、联系人、规则和安全预览继续按领域组件与独立分页拆分；联系人领域流程位于 `src/hooks/useContactManagement.ts`，OAuth2 设置与授权流程位于 `src/hooks/useOAuthFlow.ts`，写入验收持久化编排位于 `src/hooks/useProviderWriteValidation.ts`，轻量邮件拖拽协议位于 `src/components/messageDrag.ts`，领域类型和纯配置位于 `src/app/`。
