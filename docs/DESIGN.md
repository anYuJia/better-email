...
- `src/`：前端界面、状态管理、列表/阅读/撰写交互；主界面已拆分 `src/components/Sidebar.tsx`、`MessageListPane.tsx`、`ReaderPane.tsx`，轻量邮件拖拽协议位于 `src/components/messageDrag.ts`，领域类型和纯配置位于 `src/app/`。
