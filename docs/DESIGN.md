...
- 邮件解析：MIME、多部分正文、HTML 安全渲染、内嵌图片、附件；附件按 MIME part 分段下载，BODYSTRUCTURE、整封回退和分段请求最多自动尝试 3 次。安全范围内的 `.download` 临时文件在失败和应用重启后继续保留，下次下载以文件长度作为 IMAP partial offset；完成前不写入已下载状态，完成后原子重命名，阅读面持续提供进度、失败原因和手动重试入口。
