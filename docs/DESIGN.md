...
服务商验收分为只读与写入两层：只读“一键验收”自动执行服务器连接、凭据登录、文件夹发现和首轮邮件头同步，不发送邮件、不修改远端状态；写入验收默认折叠，只生成发给当前账号且带唯一验证编号的草稿。草稿没有附件、定时发送或自动提交行为，用户需要在撰写器中再次检查收件人，并按需手动添加不含敏感信息的小文件后主动发送。后续通过同一验证编号检查 SMTP 状态、IMAP Sent 留档、自发自收、附件读取和已读/星标/归档/恢复回写。
...
- 服务商兼容性矩阵已具备 UI 展示、预设复用、四步连接诊断、只读一键验收、手动确认的写入验收草稿、脱敏诊断导出和本地真实账号验证记录；网易 163 已保留历史 IMAP/SMTP TLS 与授权码认证通过样本及后续凭据拒绝样本，继续补本机凭据库复验、其他服务商和真实发送/附件结果。
