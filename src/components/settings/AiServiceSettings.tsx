    ...
    <div className="settings-experience-stack">
      <section className="tool-panel settings-ai-panel" data-settings-section="ai">
        <header className="tool-header">
          <span>
            <strong>AI 服务</strong>
            <small>配置翻译、摘要与模板生成所用的 AI 服务。</small>
          </span>
          <em>{statusLabel}</em>
        </header>
        ...
        <div className={`settings-ai-config-area${config.enabled ? '' : ' is-dimmed'}`}>
          <label className="checkbox-row settings-ai-enabled">
            ...
            <span>
              ...
              <small>关闭后翻译、摘要与模板生成不可用，已保存的配置会保留。</small>
            </span>
          </label>
          ...
          {external && (
            <>
              ...
              <div className="settings-ai-privacy settings-ai-privacy-confirm">
                <div className="settings-ai-privacy-title"><ShieldAlert size={14} /> 隐私确认</div>
                <p>
          ...
          {config.serviceType === 'mock' && (
            <div className="settings-ai-privacy">
              <div className="settings-ai-privacy-title"><FlaskConical size={14} /> 本地演示模式</div>
              <p>
        ...
        <div className="settings-ai-actions">
          <button type="button" className="secondary" onClick={runTestConnection} disabled={testing || !config.enabled}>
            <PlugZap size={14} />
            {testing ? '测试中…' : '测试连接'}
          </button>
          <button type="button" className="primary" onClick={saveConfig} disabled={saving}>
            <Globe size={14} />
            {saving ? '保存中…' : '保存设置'}
          </button>
        </div>
