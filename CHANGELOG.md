...
### UI / UX
- 重整主工作区、阅读器、设置与写信窗口的视觉层级：减少厚重边框、胶囊化控件和高饱和装饰，改用连续表面、细分隔线与克制的焦点提示。
- 写信窗口重新梳理发件账号、身份、收件人、正文、附件与高级操作；窄屏布局、暗色模式、最小化和恢复点提示保持一致。
- 设置页和快捷键帮助改为更清晰的分区与响应式布局，移除重复保存入口和冗长辅助文案。

### 修复
- 修复批量操作菜单被工具栏裁切、菜单 Escape 穿透、Snooze/上下文菜单关闭后焦点丢失，以及窄屏侧栏无法用 Escape 关闭的问题。
- 修复虚拟化会话列表行重叠导致点击被错误行拦截；加载更多后将焦点交给第一封新邮件。
- 修复搜索、清空快速回复、文件夹切换和刷新竞态中的过期状态与焦点落到页面主体的问题。
- 修复归档、移动、删除等操作刷新后错误跳离源文件夹；发送成功留在原上下文，发送失败保留当前写信内容。
- 补齐关键按钮、列表行、菜单、对话框和表单的键盘语义、焦点环与 aria 状态。

### 质量
- 增加 UI 去卡片化、响应式、焦点恢复、虚拟列表和发送流程回归覆盖；发布前通过前端测试、类型检查、CSS 图谱检查、生产构建及 Rust 校验。
