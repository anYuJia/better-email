    ...
    pub fn list_accounts(&self) -> MailResult<Vec<Account>> {
        self.with_conn(|conn| {
            let mut stmt = conn.prepare(
                "SELECT id, email, display_name, provider, imap_host, smtp_host, incoming_protocol, auth_type, sync_mode, remote_images_allowed, signature, cross_account_risk_warning, block_external_mailboxes, intercept_https_links, auto_download_attachments, fetch_history_attachments, warn_external_senders, onboarding_completed, is_default
            ...
            conn.execute(
                "UPDATE accounts
                 SET display_name = ?1, provider = ?2, imap_host = ?3, smtp_host = ?4,
                     ...
                     auto_download_attachments = ?14, fetch_history_attachments = ?15, warn_external_senders = ?16
                 WHERE id = ?10",
                params![
