...
- 首屏只同步加载三栏工作区、列表和阅读面板；`ComposerWindow`、设置框架与独立设置页、命令面板、快捷键帮助通过 `React.lazy` + `Suspense` 延迟加载，首次打开时显示统一轻量加载浮层。2026-07-11 构建样本中完整前端产物为 725.7 KB，首屏直接加载 JS + CSS 为 488.0 KB，另有 236.4 KB 低频资源按需加载；拆分前主 JS + CSS 约 619.8 KB，首屏资源下降约 21%。
